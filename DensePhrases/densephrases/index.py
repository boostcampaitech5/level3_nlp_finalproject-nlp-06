...
class MIPS(object):
    def __init__(self, phrase_dump_dir, index_path, idx2id_path, cuda=False, logging_level=logging.INFO):
        ...
        self.index = faiss.read_index(
            index_path, faiss.IO_FLAG_ONDISK_SAME_DIR)
        self.reconst_fn = faiss.downcast_index(self.index.index).reconstruct
        self.R = torch.FloatTensor(faiss.vector_to_array(faiss.downcast_VectorTransform(
            self.index.chain.at(0)).A).reshape(self.index.d, self.index.d))
        ...
        doc_group_path = os.path.join(
            self.phrase_dump_dir[:self.phrase_dump_dir.index(
                '/phrase')], 'meta_compressed.pkl'
        )
    ...
    def open_dumps(self):
        if os.path.isdir(self.phrase_dump_dir):
            self.phrase_dump_paths = sorted(
                [os.path.join(self.phrase_dump_dir, name) for name in os.listdir(
                    self.phrase_dump_dir) if 'hdf5' in name]
            )
            dump_names = [os.path.splitext(os.path.basename(path))[
                0] for path in self.phrase_dump_paths]
            if '-' in dump_names[0] and ('dev' not in dump_names[0]):  # Range check
                self.dump_ranges = [list(map(int, name.split('-')))
                                    for name in dump_names]
        else:
            self.phrase_dump_paths = [self.phrase_dump_dir]
        self.phrase_dumps = [h5py.File(path, 'r')
    ...
    def decompress_meta(self, doc_idx):
        # needed for store from binary stream
        dtypes = self.doc_groups[doc_idx]['dtypes']
        word2char_start = np.frombuffer(blosc.decompress(
            self.doc_groups[doc_idx]['word2char_start']), dtypes['word2char_start'])
        word2char_end = np.frombuffer(blosc.decompress(
            self.doc_groups[doc_idx]['word2char_end']), dtypes['word2char_end'])
        f2o_start = np.frombuffer(blosc.decompress(
            self.doc_groups[doc_idx]['f2o_start']), dtypes['f2o_start'])
        ...
        for dump_range, dump in zip(self.dump_ranges, self.phrase_dumps):
            if dump_range[0] * 1000 <= int(doc_idx) < dump_range[1] * 1000:
                if str(doc_idx) not in dump:
                    raise ValueError(
                        '%d not found in dump list' % int(doc_idx))
    ...
    def adjust_sent(self, each):
        ...
        each['context'] = ' '.join(
            [sents[sidx][0] for sidx in range(sent_idxs[0], sent_idxs[-1]+1)])
    ...
    def search_dense_dynamic(self, p_query, s_query, q_texts, nprobe=256, top_k=10):
    ...
    def search_phrase(self, query, start_doc_idxs, start_idxs, orig_start_idxs, end_doc_idxs, end_idxs, orig_end_idxs,
        ...
        query = np.reshape(np.tile(np.expand_dims(query, 1), [
                           1, top_k, 1]), [-1, query.shape[1]])
        q_idxs = np.reshape(np.tile(np.expand_dims(
            np.arange(num_queries), 1), [1, top_k*2]), [-1])
        ...
        assert len(start_doc_idxs) == len(
            start_idxs) == len(end_idxs) == len(start_scores)

        # Set default vec
        start_time = time()
        query_start, query_end = np.split(query, 2, axis=1)
        bs = query_start.shape[1]
        default_doc = [doc_idx for doc_idx in set(
            start_doc_idxs.tolist() + end_doc_idxs.tolist()) if doc_idx >= 0][0]
        ...
        # Get metadata from RAM
            ...
            # no need for dequantization when using reconstruct()
            self.dequant = lambda offset, scale, x: x
        ...
        start_time = time()
        ends = [group_start['end']
                for start_idx, group_start in zip(start_idxs, groups_start)]
        new_end_idxs = [[
            start_idx+i
        ...
        end_mask = -1e9 * (np.array(new_end_idxs) < 0)  # [Q, L]
            ...
            (query.shape[0], max_answer_length, default_vec.shape[0]), dtype=np.float32)
        for end_idx, each_end in enumerate(ends):
            end[end_idx, :each_end.shape[0], :] = self.dequant(
                float(groups_all[default_doc]['offset']), float(
                    groups_all[default_doc]['scale']), each_end
            )
        ...
        with torch.no_grad():
            ...
            new_end_scores = (query_end.unsqueeze(1) *
                              end).sum(2).cpu().numpy()
        scores1 = np.expand_dims(start_scores, 1) + \
            new_end_scores + end_mask  # [Q, L]
        pred_end_idxs = np.stack([each[idx] for each, idx in zip(
            new_end_idxs, np.argmax(scores1, 1))], 0)  # [Q]
        pred_end_vecs = np.stack([each[idx] for each, idx in zip(
            end.cpu().numpy(), np.argmax(scores1, 1))], 0)
        logger.debug(f'2) {time()-start_time:.3f}s: find end')

        # Find start fot end_idxs
        start_time = time()
        starts = [group_end['start']
                  for end_idx, group_end in zip(end_idxs, groups_end)]
        new_start_idxs = [[
            end_idx-i
        ...
        for start_idx, each_start in enumerate(starts):
            start[start_idx, -each_start.shape[0]:, :] = self.dequant(
                float(groups_all[default_doc]['offset']), float(
                    groups_all[default_doc]['scale']), each_start
            )
        ...
        with torch.no_grad():
            ...
            new_start_scores = (query_start.unsqueeze(1)
                                * start).sum(2).cpu().numpy()
        scores2 = new_start_scores + \
            np.expand_dims(end_scores, 1) + start_mask  # [Q, L]
        pred_start_idxs = np.stack([each[idx] for each, idx in zip(
            new_start_idxs, np.argmax(scores2, 1))], 0)  # [Q]
        pred_start_vecs = np.stack([each[idx] for each, idx in zip(
            start.cpu().numpy(), np.argmax(scores2, 1))], 0)
        logger.debug(f'3) {time()-start_time:.3f}s: find start')

        # Get start/end idxs of phrases
        start_time = time()
        doc_idxs = np.concatenate((np.expand_dims(
            start_doc_idxs, 1), np.expand_dims(end_doc_idxs, 1)), axis=1).flatten()
        start_idxs = np.concatenate((np.expand_dims(
            start_idxs, 1), np.expand_dims(pred_start_idxs, 1)), axis=1).flatten()
        end_idxs = np.concatenate((np.expand_dims(
            pred_end_idxs, 1), np.expand_dims(end_idxs, 1)), axis=1).flatten()
        max_scores = np.concatenate((np.max(scores1, 1, keepdims=True), np.max(
            scores2, 1, keepdims=True)), axis=1).flatten()
        ...
        for i in range(len(new_out)):
            new_out[i] = sorted(
                new_out[i], key=lambda each_out: -each_out['score'])
            # In case of no output but masks
            new_out[i] = list(filter(lambda x: x['score'] > -1e5, new_out[i]))
        logger.debug(f'4) {time()-start_time:.3f}s: get metadata')
        return new_out

    def search_phrase_dynamic(self, p_query, s_query, start_doc_idxs, start_idxs, orig_start_idxs, end_doc_idxs, end_idxs, orig_end_idxs,
    ...
    def aggregate_results(self, results, top_k=10, q_text=None, agg_strat='opt1'):
        ...
        for r_idx, result in enumerate(results):
            if agg_strat == 'opt0': # for dynamic retrieval
                ...
                da = f'{result["title"]}'
            # for aggregate with answer and merge titles (for KILT)
            elif agg_strat == 'opt4':
        ...
        # not exactly top-k but will be cut during evaluation
        results = list(filter(lambda x: x['score'] > -1e5, results))
        return results
    ...
    def search(self, query, q_texts=None,
        ...
        # MIPS on start/end
        start_time = time()
        if len(query) > 1: # for dynamic retrieval
            p_query, s_query, start_doc_idxs, start_idxs, start_I, end_doc_idxs, end_idxs, end_I, start_scores, end_scores, start_unit, end_unit = self.search_dense_dynamic(
                query[0], 
                query[1],
                q_texts=q_texts,
                nprobe=nprobe,
        ...
        start_time = time()
        if len(query) > 1: # for dynamic retrieval
            outs = self.search_phrase_dynamic(
                query[0], query[1], start_doc_idxs, start_idxs, start_I, end_doc_idxs, end_idxs, end_I, start_scores, end_scores,
            ...
            outs = self.search_phrase(
                query[0], start_doc_idxs, start_idxs, start_I, end_doc_idxs, end_idxs, end_I, start_scores, end_scores,
                top_k=top_k, max_answer_length=max_answer_length, return_idxs=return_idxs, return_sent=return_sent
            )
