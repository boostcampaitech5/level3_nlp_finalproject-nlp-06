...
class Options():
    ...
    def add_index_options(self):
        ...
        self.parser.add_argument(
            '--trained_index_path', default='trained.faiss')
        ...
        self.parser.add_argument(
            '--add_all', action='store_true', default=False)
        ...
        self.parser.add_argument('--fine_quant', default='SQ4',
                                 help='SQ4|OPQ# where # is number of bytes per vector')
        ...
        self.parser.add_argument(
            '--first_passage', action='store_true', default=False, help="dump only first passages")
        self.parser.add_argument(
            "--index_filter", type=float, default=-1e8, help="filtering threshold for index",)
        ...
        self.parser.add_argument(
            '--subindex_name', default='index', help='used only if dump_path is specified.')
    ...
    def add_data_options(self):
        ...
        self.parser.add_argument(
            "--truecase_path", type=str, default='truecase/english_with_questions.dist')
        self.parser.add_argument(
            "--truecase", action="store_true", help="Dummy (automatic truecasing supported)")
    ...
    def add_rc_options(self):
        ...
        self.parser.add_argument(
            "--do_filter_test", action="store_true", help="Whether to test filters.")
        self.parser.add_argument(
            "--lambda_kl", default=0.0, type=float, help="Lambda for distillation")
        self.parser.add_argument(
            "--lambda_neg", default=0.0, type=float, help="Lambda for in-batch negative")
        self.parser.add_argument(
            "--lambda_flt", default=0.0, type=float, help="Lambda for filtering")
        self.parser.add_argument(
            "--pbn_size", default=0, type=int, help="pre-batch negative size")
        self.parser.add_argument(
            "--pbn_tolerance", default=9999, type=int, help="pre-batch tolerance epoch")
        self.parser.add_argument(
            "--append_title", action="store_true", help="Whether to append title in context.")
        ...
        self.parser.add_argument(
            "--weight_decay", type=float, default=0.0, help="Weight decay if we apply some.")
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--logging_steps", type=int, default=5000, help="Log every X updates steps.")
        self.parser.add_argument(
            "--save_steps", type=int, default=9999999, help="Save checkpoint every X updates steps.")
        ...
        self.parser.add_argument(
            "--wandb", action="store_true", help="Whether to use Weights and Biases logging")
        ...
        self.parser.add_argument(
            "--do_dump", action="store_true", help="Whether to generate phrase vecs")
        self.parser.add_argument(
            "--filter_only", action="store_true", help="Whether to generate filter logits only")
        self.parser.add_argument('--dense_offset', type=float, default=-2)
        self.parser.add_argument('--dense_scale', type=float, default=20)
        self.parser.add_argument("--filter_threshold", type=float,
                                 default=-1e8, help="model-based filtering threshold.",)
    ...
    def add_retrieval_options(self):
        ...
        # Evaluation
        self.parser.add_argument(
            '--dev_path', default='open-qa/nq-open/dev_preprocessed.json')
        self.parser.add_argument(
            '--test_path', default='open-qa/nq-open/test_preprocessed.json')
        ...
        self.parser.add_argument(
            '--save_pred', action='store_true', default=False)

        # Passage retrieval options
        self.parser.add_argument(
            '--eval_psg', action='store_true', default=False)
        self.parser.add_argument('--psg_top_k', type=int, default=100)
        self.parser.add_argument(
            '--max_psg_len', type=int, default=999999999, help='used for fair comparison')
        self.parser.add_argument(
            '--mark_phrase', action='store_true', default=False)
        self.parser.add_argument(
            '--return_sent', action='store_true', default=False)
        self.parser.add_argument('--sent_window', type=int, default=0)

        # KILT
        self.parser.add_argument(
            '--is_kilt', action='store_true', default=False)
        self.parser.add_argument(
            '--kilt_gold_path', default='kilt/trex/trex-dev-kilt.jsonl')
        self.parser.add_argument(
            '--title2wikiid_path', default='wikidump/title2wikiid.json')

    # Query-side fine-tuning options
    def add_qsft_options(self):
        self.parser.add_argument('--train_path', default=None)
        self.parser.add_argument(
            '--per_gpu_train_batch_size', default=48, type=int)
        ...
        self.parser.add_argument('--label_strat', default='phrase',
                                 type=str, help="label strat={phrase|doc|sentence|context}")
        self.parser.add_argument('--add_component', type=bool, default=False)
    ...
    def parse(self):
        ...
        if opt.draft:
            ...
            logger.warning(
                f'Overwrite model in {opt.output_dir} in draft version')
